...
use super::{error::ERROR_MARGIN, utils::rotate_90_degree_clockwise};
use geometric_algebra::{
    epga1d,
    polynomial::{solve_cubic, solve_linear, solve_quadratic, solve_quartic, Root},
    ppga2d, ppga3d, Dual, GeometricProduct, GeometricQuotient, InnerProduct, Powf, Powi,
    RegressiveProduct, Signum, Zero,
};
...
pub fn rational_quadratic_control_points_to_power_basis(
    control_points: &[ppga2d::Point; 3],
) -> [ppga2d::Point; 3] {
    [
...
pub fn rational_cubic_control_points_to_power_basis(
...
pub fn reparametrize_rational_quadratic(
...
///
/// Can be used for splitting, trimming and bloosoming.
pub fn reparametrize_rational_cubic(
...
pub fn rational_quadratic_first_order_derivative(
    power_basis: &[ppga2d::Point; 3],
    t: f32,
) -> ppga2d::Plane {
...
pub fn rational_quadratic_second_order_derivative(
    power_basis: &[ppga2d::Point; 3],
    t: f32,
) -> ppga2d::Plane {
    let p = mat_vec_transform!(power_basis, 1.0, t, t.powi(2));
...
pub fn rational_cubic_first_order_derivative(
    power_basis: &[ppga2d::Point; 4],
    t: f32,
) -> ppga2d::Plane {
...
pub fn rational_cubic_second_order_derivative(
    power_basis: &[ppga2d::Point; 4],
    t: f32,
) -> ppga2d::Plane {
    let p = mat_vec_transform!(power_basis, 1.0, t, t.powi(2), t.powi(3));
...
pub fn inflection_point_polynomial_coefficients(
    power_basis: &[ppga2d::Point; 4],
    integral: bool,
) -> [f32; 4] {
...
pub fn integral_inflection_points(
    ippc: &[f32; 4],
    loop_self_intersection: bool,
) -> (f32, [Root; 3]) {
...
/// In case there is a loop (discriminant < 0.0) and the `loop_self_intersection` flag is [true],
/// two different roots will be located at the self intersection point.
pub fn rational_inflection_points(
    ippc: &[f32; 4],
    loop_self_intersection: bool,
) -> (f32, [Root; 3]) {
    ...
    let (discriminant, roots, real_root) = solve_cubic(
        [ippc[3] * -1.0, ippc[2] * 3.0, ippc[1] * -3.0, ippc[0]],
        ERROR_MARGIN,
    );
    ...
    ($start_tangent:expr, $end_tangent:expr, $angle_step:expr, $normal:ident, $solutions:expr $(,)?) => {{
        ...
        (1..steps)
            .map(|i| {
                ...
                let $normal =
                    ppga2d::Plane::new(0.0, interpolated.real(), interpolated.imaginary());
...
pub fn integral_cubic_uniform_tangent_angle(
    power_basis: &[ppga2d::Point; 4],
    angle_step: f32,
) -> Vec<f32> {
...
pub fn rational_cubic_uniform_tangent_angle(
    power_basis: &[ppga2d::Point; 4],
    angle_step: f32,
) -> Vec<f32> {
