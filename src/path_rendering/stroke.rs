use super::{
    curve::{
        integral_cubic_uniform_tangent_angle, integral_quadratic_uniform_tangent_angle,
        ...
        rational_quadratic_control_points_to_power_basis,
        rational_quadratic_first_order_derivative, rational_quadratic_point,
    ...
    utils::{
        line_line_intersection, point_to_vec, rotate_90_degree_clockwise, vec_to_point,
        weighted_vec_to_point,
    },
    vertex::{Vertex2f1i, Vertex3f1i},
};
use geometric_algebra::{
    ppga2d, Dual, GeometricProduct, GeometricQuotient, InnerProduct, Magnitude, OuterProduct,
    RegressiveProduct, Signum, SquaredMagnitude, Zero,
};

fn offset_control_point(
...
fn emit_stroke_vertex(
    path_line_vertices: &mut Vec<Vertex2f1i>,
    path_index: usize,
    offset_along_path: f32,
    vertex: ppga2d::Point,
    side: f32,
) {
    path_line_vertices.push(Vertex2f1i(
        point_to_vec(vertex),
        [side, offset_along_path],
        path_index as u32,
    ));
}
...
fn emit_stroke_vertices(
    ...
    emit_stroke_vertex(
        ...
        path_index,
        offset_along_path,
        offset_control_point(
            point,
            tangent,
            (stroke_options.offset.unwrap() - 0.5) * stroke_options.width.unwrap(),
        ),
        -0.5,
    );
    emit_stroke_vertex(
...
fn emit_stroke_join(
    ...
    let side_offset =
        (stroke_options.offset.unwrap() - side_sign * 0.5) * stroke_options.width.unwrap();
    ...
    let previous_edge_tangent: ppga2d::Plane = previous_tangent
        ...
        .into();
    let next_edge_tangent: ppga2d::Plane = next_tangent
        .inner_product(next_edge_vertex)
        .geometric_product(next_edge_vertex)
        .into();
    let intersection = line_line_intersection(previous_edge_tangent, next_edge_tangent);
    let mut vertices = [
        control_point,
        previous_edge_vertex,
        next_edge_vertex,
        intersection,
        intersection,
    ];
    ...
    if anti_parallel || control_point.regressive_product(intersection).magnitude() > miter_clip {
        ...
        let clipping_vertex =
            offset_control_point(control_point, mid_tangent, -side_sign * miter_clip);
        let clipping_plane: ppga2d::Plane = mid_tangent
            .inner_product(clipping_vertex)
            .geometric_product(clipping_vertex)
            .into();
    ...
    for vertex in vertices.iter() {
        builder.joint_vertices.push(Vertex3f1i(
            ...
            [
                ...
                vertex
                    .regressive_product(control_point)
                    .inner_product(scaled_tangent),
                offset_along_path,
            ],
    ...
    let mut indices: Vec<u16> =
        (start_index as u16..(builder.joint_vertices.len() + 1) as u16).collect();
    *indices.iter_mut().last().unwrap() = (-1isize) as u16;
    builder.joint_indices.append(&mut indices);
    *length_accumulator +=
        tangets_dot_product.acos() / (std::f32::consts::PI * 2.0) * stroke_options.width.unwrap();
    ...
    if !builder.path_line_vertices.is_empty() {
        proto_hull.append(
            &mut builder
                .path_line_vertices
                .iter()
                .map(|vertex| vertex.0.into())
                .collect(),
        );
        let start_index = builder.line_vertices.len();
        builder
            .line_vertices
            .append(&mut builder.path_line_vertices);
        let mut indices: Vec<u16> =
            (start_index as u16..(builder.line_vertices.len() + 1) as u16).collect();
        *indices.iter_mut().last().unwrap() = (-1isize) as u16;
    ...
    ($builder:expr, $stroke_options:expr, $length_accumulator:expr,
     ...
     $point:ident, $tangent:ident $(,)?) => {
        let parameters: Vec<f32> = match $stroke_options.curve_approximation {
            CurveApproximation::UniformlySpacedParameters(steps) => {
                (1..steps + 1).map(|i| i as f32 / steps as f32).collect()
            }
...
fn get_quadratic_tangents(control_points: &[ppga2d::Point; 3]) -> (ppga2d::Plane, ppga2d::Plane) {
    let mut segment_start_tangent = control_points[0]
        .regressive_product(control_points[1])
        .signum();
    let mut segment_end_tangent = control_points[1]
        .regressive_product(control_points[2])
        .signum();
    if segment_start_tangent[0].is_nan() || segment_end_tangent[0].is_nan() {
        segment_start_tangent = control_points[0]
            .regressive_product(control_points[2])
            .signum();
...
fn get_cubic_tangents(control_points: &[ppga2d::Point; 4]) -> (ppga2d::Plane, ppga2d::Plane) {
    ...
    let mut segment_end_tangent = control_points[2]
        .regressive_product(control_points[3])
        .signum();
    if segment_end_tangent[0].is_nan() {
        segment_end_tangent = control_points[1]
            .regressive_product(control_points[3])
            .signum();
    }
    if segment_start_tangent[0].is_nan() || segment_end_tangent[0].is_nan() {
        segment_end_tangent = control_points[0]
            .regressive_product(control_points[3])
            .signum();
    }
    (segment_start_tangent, segment_end_tangent)
...
impl StrokeBuilder {
    pub fn add_path(
        ...
        let mut integral_quadratic_curve_segment_iter =
            path.integral_quadratic_curve_segments.iter().peekable();
        let mut integral_cubic_curve_segment_iter =
            path.integral_cubic_curve_segments.iter().peekable();
        let mut rational_quadratic_curve_segment_iter =
            path.rational_quadratic_curve_segments.iter().peekable();
        let mut rational_cubic_curve_segment_iter =
            path.rational_cubic_curve_segments.iter().peekable();
        ...
        for segment_type in path.segment_types.iter() {
                ...
                if !stroke_options.closed {
                    ...
                    emit_stroke_vertices(
                        ...
                        offset_control_point(
                            previous_control_point,
                            normal,
                            0.5 * stroke_options.width.unwrap().abs(),
                        ),
            ...
            match segment_type {
                SegmentType::Line => {
                    length_accumulator += previous_control_point
                        .regressive_product(next_control_point)
                        .magnitude();
                    emit_stroke_vertices(
                        self,
                ...
                SegmentType::IntegralQuadraticCurve => {
                    ...
                    emit_curve_stroke!(
                        ...
                        power_basis,
                        angle_step,
                        integral_quadratic_uniform_tangent_angle(
                            &power_basis,
                            segment_start_tangent,
                            segment_end_tangent,
                            angle_step.unwrap()
        ...
        if stroke_options.closed {
            let line_segment =
                previous_control_point.regressive_product(vec_to_point(path.start.unwrap()));
